...
def load_material_data(filename: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    1. Two sections: wavelength (μm), real(n) and wavelength (μm), imag(n)
    2. Single section: wavelength (μm), real(n) (with k=0)
    """
    ...
    n_data = []
    for i in range(1, separator_idx if separator_idx != -1 else len(lines)):  # Skip header
    ...
    if separator_idx == -1:
        # No separator found - assume only real refractive index (k=0)
        print("No k data found, assuming k=0 (no absorption)")
        wavelengths = n_data[:, 0]  # in μm
        n_complex = n_data[:, 1] + 1j * 0.0  # k = 0
...
if __name__ == "__main__":
    ...
    wavelengths, n_complex = load_material_data("Material_TiO2.csv")
    ...
    num_lorentzians = 2  # Increased for better fit of metal data

    # Number of times to repeat local optimization with random initial values.
    num_repeat = 50  # Increased for better optimization
    ...
    fig.savefig("eps_fit_TiO2.png", dpi=150, bbox_inches="tight")
    print("Plot saved as 'eps_fit_TiO2.png'")
